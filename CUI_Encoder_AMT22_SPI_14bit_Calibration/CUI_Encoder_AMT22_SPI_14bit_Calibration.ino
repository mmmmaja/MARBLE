 ...
 * 
 * Arduino UNO and Mega () Pin Connections
 * SPI Chip Select Enc 0:   Pin  2 (5)
 * SPI Chip Select Enc 1:   Pin  3 (6)
 * SPI MOSI                 Pin 11 (51)
 * SPI MISO                 Pin 12 (50)
 * SPI SCLK:                Pin 13 (52)
 * 
...
/* SPI pins */
#define ENC_0            5 //assessment device pin/cs
#define ENC_1            6 //orthosis pin/cs
#define SPI_MOSI        51
#define SPI_MISO        50
#define SPI_SCLK        52
...
uint16_t minPosition[2]={0,0}; //minimum angle (needed for encoder calibration)
uint16_t maxPosition[2]={0,0}; //maximum angle (needed for encoder calibration)
uint16_t currentPosRaw[2]={0,0};
uint16_t currentPosDeg[2]={0,0};
...
void setup() 
{
  ...
  Serial.println("Put orthosis and assessment device in zero degrees position");
  delay(5000);
  //setZeroSPI(ENC_0);//set current position as the zero position
  //setZeroSPI(ENC_1);//set current position as the zero position 
  minPosition[0] = getPositionSPI(ENC_0, RES14); //read current encoder value from assessment device
  minPosition[1] = getPositionSPI(ENC_1, RES14); //read current encoder value from orthoses
  if(minPosition[0] == 0xFFFF)
    {
      Serial.println("Error!!!!!!!! Assessment device encoder can not be found!!!!!!!");
    }
  if(minPosition[1] == 0xFFFF)
    {
      Serial.println("Error!!!!!!!! Orthosis encoder can not be found!!!!!!!!");
    }
  Serial.println("---------------------------------------");
  Serial.println("Put orthosis and assessment device in 90 degrees position");
  delay(5000);
  maxPosition[0] = getPositionSPI(ENC_0, RES14); 
  maxPosition[1] = getPositionSPI(ENC_1, RES14); 
  if(maxPosition[0] == 0xFFFF)
    {
      Serial.println("Error!!!!!!!! Assessment device encoder can not be found!!!!!!!");
    }
  if(maxPosition[1] == 0xFFFF)
    {
      Serial.println("Error!!!!!!!! Orthosis encoder can not be found!!!!!!!!");
    }
  Serial.println("---------------------------------------");
  Serial.println("Min and max position of assessment device:");
  Serial.print("Min Position: ");
  Serial.println(minPosition[0]);
  Serial.print("Max Position: ");
  Serial.println(maxPosition[0]);
  Serial.println("---------------------------------------");
  Serial.println("Min and max position of orthosis:");
  Serial.print("Min Position: ");
  Serial.println(minPosition[1]);
  Serial.print("Max Position: ");
...
void loop() 
{
  ...
  currentPosRaw[0] = getPositionSPI(ENC_0, RES14); //read assessment device angle
  currentPosRaw[1] = getPositionSPI(ENC_1, RES14); //read orthosis angle
  Serial.print("Current Position Assessment Device (raw): ");
  Serial.println(currentPosRaw[0]);
  Serial.print("Current Position Orthosis (raw): ");
  Serial.println(currentPosRaw[1]);
  Serial.println(minRange);
  Serial.println(maxRange);
  //map digital encoder values to angles in degrees:
  currentPosDeg[0]=map(currentPosRaw[0],minPosition[0],maxPosition[0],minRange,maxRange);
  currentPosDeg[1]=map(currentPosRaw[1],minPosition[1],maxPosition[1],minRange,maxRange);
  Serial.print("Current Position Assessment Device (degs): ");
  Serial.println(currentPosDeg[0]);
  Serial.print("Current Position Orthosis (degs): ");
  Serial.println(currentPosDeg[1]);
  

  //For Plotting with the serial monitor comment prevous serial prints and uncomment following lines
//  Serial.println(0);
//  Serial.print(" ");
